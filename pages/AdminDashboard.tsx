...
const AdminDashboard: React.FC = () => {
  ...
  const generateDailyPosts = async () => {
    ...
    setLogs([]);
    addLog("Iniciando Protocolo CMBDIGITAL (Engine: OpenAI GPT-4o)...");

    try {
      addLog("Solicitando curadoria ao servidor backend...");
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Falha na comunicação com o motor editorial.");
      }

      const data = await response.json();
      const newArticles = data.articles || [];

      addLog(`Sucesso: ${newArticles.length} rascunhos de alta qualidade gerados.`);
      
      const updatedDrafts = [...newArticles, ...drafts];
      setDrafts(updatedDrafts);
      localStorage.setItem('cmb_drafts', JSON.stringify(updatedDrafts));
      
      addLog("PRONTO: Conteúdo aguardando revisão na fila de rascunhos.");
  ...
  const publishArticle = (id: string) => {
    ...
    // Converter o conteúdo de texto puro para parágrafos HTML para exibição no blog
    const htmlContent = articleToPublish.content
      .split('\n\n')
      .map(p => `<p>${p.trim()}</p>`)
      .join('');

    const published = JSON.parse(localStorage.getItem('cmb_published') || '[]');
    localStorage.setItem('cmb_published', JSON.stringify([
      { ...articleToPublish, content: htmlContent, status: 'published' }, 
      ...published
    ]));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-obsidian px-4">
        <div className="w-full max-w-md text-center">
          <h1 className="text-4xl font-black text-brand-soft uppercase tracking-tighter mb-8">Painel Editorial</h1>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div>
            ...
            <p className="text-brand-muted font-mono text-[10px] tracking-widest">ENGINE: OPENAI GPT-4o | STATUS: {isGenerating ? 'EXECUTANDO...' : 'READY'}</p>
          </div>
          <div className="flex gap-4">
             ...
             >
               {isGenerating ? 'Processando...' : 'Iniciar Varredura Diária'}
             </button>
          </div>
        </div>

        {/* Terminal de Logs */}
        <div className="mb-12 p-8 rounded-[2rem] bg-black/40 border border-brand-graphite/50 font-mono text-[11px] text-brand-cyan min-h-[200px] shadow-inner">
          {logs.length === 0 ? (
            <div className="opacity-40 italic">> Terminal operacional. Pressione "Iniciar Varredura Diária" para gerar conteúdo via OpenAI.</div>
          ) : (
        ...
        <div className="space-y-10">
          ...
          {drafts.length === 0 ? (
            <div className="py-24 text-center border-2 border-dashed border-brand-graphite rounded-[3rem] text-brand-muted italic">
              Nenhum conteúdo pendente na fila.
            </div>
          ) : (
              ...
              {drafts.map(draft => (
                <div key={draft.id} className="p-8 rounded-[3rem] bg-brand-graphite/20 border border-brand-graphite flex flex-col md:flex-row gap-10 hover:border-brand-cyan/40 transition-all group">
                  <div className="md:w-64 h-48 rounded-[2rem] overflow-hidden border border-brand-graphite shrink-0 bg-brand-obsidian">
                    <img src={draft.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 opacity-80" alt="Preview" />
                  </div>
